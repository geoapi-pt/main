...
module.exports = {
  fn: routeFn,
  route: '/municipios?/:municipality?/freguesias?/:parish?/subsec(c|ç|%C3%A7)o(e|ẽ|%E1%BA%BD)s'
}
...
async function routeFn (req, res, next, { administrations, regions }) {
  ...
  if (parishResults.length === 1) {
    ...
    let subsectionsGeojson, filteredSubsectionsGeojson
    // eslint-disable-next-line prefer-const
    subsectionsGeojson = filteredSubsectionsGeojson = JSON.parse(fs.readFileSync(geojsonFile, 'utf-8'))

    filteredSubsectionsGeojson.features = subsectionsGeojson.features
      .filter(el => el.properties.DTMNFR21.padStart(6, '0') === parishCode)

    if (filteredSubsectionsGeojson.features.length) {
      sectionsObj.geojson = filteredSubsectionsGeojson
      sectionsObj.geojson.bbox = turf.bbox(filteredSubsectionsGeojson)
    } else {
      res.status(404).sendData({
        error: `Subsecções estatísticas não encontradas para a freguesia ${parish.nome} do município ${municipality.nome}!`
      })
      return
    }
    ...
    if (isResponseJson(req)) {
      ...
      res.status(200).sendData({
        ...
        pageTitle: `Subsecções estatísticas da Freguesia ${parish.nomecompleto} (${parish.municipio})`,
        template: 'routes/parishSubsections'
      })
    }
  } else {
    res.status(404).sendData({ error: `Subsecções não encontradas para a freguesia ${parish.nome} do município ${municipality.nome}!` })
  }
}
